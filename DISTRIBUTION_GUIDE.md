# Smart QP Print Manager - Distribution Build Guide (V5.3+)
...
```
*Note: This generates `target/java-print-manager-5.3.jar`.*
...
Copy-Item target\java-print-manager-5.3.jar jpackage_input\
```
...
jpackage --type app-image --name "Smart QP Print Manager" --app-version 5.3 --input jpackage_input --main-jar java-print-manager-5.3.jar --main-class com.printmanager.Launcher --dest output --icon src\main\resources\icon.ico
```
...
#### Native ZIP
```powershell
powershell.exe -NoProfile -Command "Compress-Archive -Path 'output\Smart QP Print Manager\*' -DestinationPath 'Smart QP Print Manager-Native-V5.3.zip' -Force"
```
...
jpackage --type msi --name "Smart QP Print Manager" --app-version 5.3 --app-image "output\Smart QP Print Manager" --dest output --icon src\main\resources\icon.ico --win-shortcut --win-menu --win-dir-chooser --win-upgrade-uuid f1a8c9e5-a6b7-4c8d-9e0f-e62c1a234b5d
Copy-Item "output\Smart QP Print Manager-5.3.msi" "Smart QP Print Manager-V5.3.msi"
```
